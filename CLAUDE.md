...
2. **Closing a circuit in position, not just in height.** The layouts close in *height* and have never been closed in position, heading and roll, so the ride's "lap" is a teleport from the last block back to the station. That is honest rather than broken — the signalling sees the jump and interlocks it — but a real wrap is `S -= TotalLength` and nothing else, once `TrackClose` is solving position and heading too.
