...
2. **A lap inside `FTrain`.** The two-train preset is now a genuinely closed circuit — 0.000000 m of position, 0.000084° of heading, 0.000000° of roll — but `FTrain` still clamps at the end of the track, so the actor returns a train to the station by teleporting it. With the seam closed to zero that teleport is *invisible*, so this is tidiness rather than a visible defect. Doing it properly is `S -= TotalLength` plus three things that are not free: `FTrack` has to say it is a circuit, `AdvanceSamples` walks incrementally and would step backwards a whole lap across the seam, and `FRideSignals::Update` currently swaps a rear/front pair that arrives reversed — on a circuit that means the train straddles the seam and holds **two** blocks, not the whole ring.
